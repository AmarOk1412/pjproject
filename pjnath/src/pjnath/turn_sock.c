...
enum {
  CONNECTION_USED = (1 << 0), /* TCP connection slot is used or free */
										  ...
										  pj_status_t status)
{
	...
	PJ_LOG(3, ("rfc6062", "peer data connection %s", status == PJ_SUCCESS
		? "ready" : "failed"));
	...
	pj_stun_session_set_credential(tcp_cnx->stun_sess,
		PJ_STUN_AUTH_LONG_TERM, &turn_sock->cred);
	...
	status = pj_stun_session_send_msg(tcp_cnx->stun_sess, tcp_cnx,
									  PJ_FALSE, PJ_FALSE,
									  &tcp_cnx->peer_addr,
									  tcp_cnx->peer_addr_len,
									  tdata);
