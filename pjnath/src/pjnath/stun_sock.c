...
pj_bool_t on_outgoing_connected(pj_activesock_t *asock, pj_status_t status); // TODO (sblin) change name
...
static pj_bool_t on_stun_sock_accept(pj_activesock_t *asock, pj_sock_t sock,
    ...
    stun_sock = (pj_stun_sock *)pj_activesock_get_user_data(asock);
    stun_sock->incoming_addr = src_addr;
    stun_sock->incoming_addr_len = src_addr_len;
    stun_sock->incoming_sock_fd = sock;
    ...
    status = pj_activesock_create(stun_sock->pool, stun_sock->incoming_sock_fd,
                                  sock_type, &activesock_cfg,
                                  stun_sock->cfg.ioqueue, &activesock_cb,
                                  stun_sock, &stun_sock->incoming_sock);
    ...
    /* Start asynchronous read operations */
    pj_status_t result =
        pj_activesock_start_read(stun_sock->incoming_sock, stun_sock->pool,
...
pj_stun_sock_connect_active(pj_stun_sock *stun_sock, pj_sockaddr *remote_addr) {
  ...
  /* Create socket and bind socket */
  status =
      pj_sock_socket(stun_sock->af, sock_type, 0, &stun_sock->outgoing_sock_fd);
  if (status != PJ_SUCCESS) {
    pj_stun_sock_destroy(stun_sock);
    pj_grp_lock_release(stun_sock->grp_lock);
    return status;
  ...
  /* Apply QoS, if specified */
  status = pj_sock_apply_qos2(
      stun_sock->outgoing_sock_fd, stun_sock->setting.qos_type,
      &stun_sock->setting.qos_params, 2, stun_sock->obj_name, NULL);
  if (status != PJ_SUCCESS && !stun_sock->setting.qos_ignore_error) {
    pj_stun_sock_destroy(stun_sock);
    pj_grp_lock_release(stun_sock->grp_lock);
  ...
  if (stun_sock->setting.so_rcvbuf_size > 0) {
    unsigned sobuf_size = stun_sock->setting.so_rcvbuf_size;
    status = pj_sock_setsockopt_sobuf(stun_sock->outgoing_sock_fd,
                                      pj_SO_RCVBUF(), PJ_TRUE, &sobuf_size);
    if (status != PJ_SUCCESS) {
      pj_perror(3, stun_sock->obj_name, status, "Failed setting SO_RCVBUF");
    } else {
      if (sobuf_size < stun_sock->setting.so_rcvbuf_size) {
  ...
  if (stun_sock->setting.so_sndbuf_size > 0) {
    unsigned sobuf_size = stun_sock->setting.so_sndbuf_size;
    status = pj_sock_setsockopt_sobuf(stun_sock->outgoing_sock_fd,
                                      pj_SO_SNDBUF(), PJ_TRUE, &sobuf_size);
    if (status != PJ_SUCCESS) {
      pj_perror(3, stun_sock->obj_name, status, "Failed setting SO_SNDBUF");
    } else {
      if (sobuf_size < stun_sock->setting.so_sndbuf_size) {
  ...
  /* Init active socket configuration */
  {
    ...
    status = pj_activesock_create(stun_sock->pool, stun_sock->outgoing_sock_fd,
                                  sock_type, &activesock_cfg,
                                  stun_sock->cfg.ioqueue, &activesock_cb,
                                  stun_sock, &stun_sock->outgoing_sock);
    ...
    status =
        pj_activesock_start_connect(stun_sock->outgoing_sock, stun_sock->pool,
                                    remote_addr, PJ_INET6_ADDRSTRLEN + 10);
    if (status == PJ_SUCCESS) {
      on_outgoing_connected(stun_sock->outgoing_sock, status);
    } else if (status != PJ_EPENDING) {
      char addrinfo[PJ_INET6_ADDRSTRLEN + 10];
      pj_perror(3, stun_sock->pool->obj_name, status, "Failed to connect to %s",
                pj_sockaddr_print(remote_addr, addrinfo, sizeof(addrinfo), 3));
      pj_stun_sock_destroy(stun_sock);
      pj_grp_lock_release(stun_sock->grp_lock);
      return;
    }
...
pj_bool_t on_outgoing_connected(pj_activesock_t *asock, pj_status_t status) {
  pj_stun_sock *stun_sock;
  stun_sock = (pj_stun_sock *)pj_activesock_get_user_data(asock);
