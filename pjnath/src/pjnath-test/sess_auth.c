			     ...
			     pj_bool_t use_ipv6)
{
    ...
    status = pj_stun_session_create(&stun_cfg, "server", &sess_cb, PJ_FALSE, NULL, &server->sess, PJ_STUN_TP_UDP);
    if (status != PJ_SUCCESS) {
	destroy_server();
	return -10;
    }
			   ...
			   int (*more_check)(void))
{
    ...
    status = pj_stun_session_create(&stun_cfg, "client", &sess_cb, PJ_FALSE, NULL, &client->sess, PJ_STUN_TP_UDP);
    if (status != PJ_SUCCESS) {
